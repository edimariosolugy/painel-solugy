...
function mesclar(live) {
  if (!live) return D0;
  const d = {...D0};
  Object.entries(BLOCOS_VIVOS).forEach(([k, ok]) => {
    if (live[k] !== undefined && live[k] !== null && ok(live[k])) d[k] = live[k];
  });
  return d;
}
  ...
  return (
    <>
      ...
      <div className="ex-disabled-grid">
        ...
        <Disabled t="Giro de estoque / DIO" why="Saldo real já está na aba Estoque; o DIO consolidado entra na próxima rodada do extrator."/>
      </div>
    </>
  );
}
...
function TabCli({D}) {
  const cl = D.clientes;
  const ins = motor(D).filter(x=>x.cat==="Clientes");
...
function TabProd({D}) {
  const pr = D.produtos;
  const ins = motor(D).filter(x=>x.cat==="Produtos");
  ...
  return (
    <>
      ...
      <div className="ex-disabled-grid">
        <Disabled t="Estoque crítico / encalhado" why="Disponível na aba Estoque — saldo real da tabela consolidada do LJ, com capital imobilizado e encalhados."/>
        <Disabled t="Giro de estoque por categoria" why="Próxima rodada do extrator: cruzar saldo (aba Estoque) com a venda por categoria."/>
      </div>
    </>
  );
}
...
function TabVend({D}) {
  ...
  const ins = motor(D).filter(x=>x.cat==="Vendedores");
...
function TabCaixa({D}) {
  const cx = D.caixa;
  const ins = motor(D).filter(x=>x.cat==="Caixa");
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <button
              onClick={()=>{
                ...
                a.href = "estoque_minimo.xlsx?t=" + Date.now();
                a.download = "estoque_minimo.xlsx";
                document.body.appendChild(a); a.click(); a.remove();
              }}
              title="Baixar planilha de estoque mínimo (atualizada 1x/dia pelo extrator)"
              style={{
                background:"transparent", border:"1px solid "+C.orange, color:C.orange,
                fontFamily:"inherit", fontSize:11, fontWeight:600, padding:"5px 10px",
...
export default function DashboardExec() {
  ...
  const [live, setLive] = useState(null);   // JSON completo do extrator
  useEffect(()=>{
    let vivo=true;
    fetch("dash_data.json?t="+Date.now(),{cache:"no-store"})
      .then(r=>r.ok?r.json():null)
      .then(j=>{ if(vivo&&j) setLive(j); })
      .catch(()=>{});
    return ()=>{vivo=false;};
  },[]);
  const D = useMemo(()=>mesclar(live), [live]);       // vivo, com fallback por bloco
  const hoje = live && live.hoje ? live.hoje : null;  // card do dia (só com JSON novo)
  useEffect(()=>{
    let vivo=true;
  ...
  const win = useMemo(()=> D.serie.slice(-jan).map(s=>s.k), [jan, D]);
  return (
    <div className="ex-root">
      ...
      <div className="ex-topbar">
        ...
        <div className="ex-topright">
          ...
          <div className="ex-sync">sinc. {(live && live.meta && live.meta.atualizado_em) || D.meta.atualizado_em}{!live && " · offline (dados 04/07)"}</div>
        </div>
      </div>
      ...
      <div className="ex-body">
        {aba==="exec" && <TabExec D={D} win={win} hoje={hoje}/>}
        {aba==="ind" && <TabInd D={D}/>}
        {aba==="cli" && <TabCli D={D}/>}
        {aba==="prod" && <TabProd D={D}/>}
        {aba==="estoque" && <TabEstoque D={D}/>}
